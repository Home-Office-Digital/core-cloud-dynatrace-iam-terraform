// TEST: Assert planned `dynatrace_iam_permission` resources match the flattened size of
// `environment_permissions` (plan-only), one per group/permission/environment combination.
// Why: prevents regressions in the legacy environment-scoped permission wiring (e.g.
// tenant-manage-support-tickets) and ensures tests run offline by skipping provider lookups.
